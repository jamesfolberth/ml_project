...
def PickBestAnswer(answerlist,percentages,x):
    ...
    for x,i in zip([a,b,c,d],range(4)):
        try:
            ...
            percent[i]=0.00000
    normpercent=percent/sum(percent)
...
if crossval==0:
    ...
    topicdict=pickle.load(open("derek_topic_prob_dict.pkl","rb"))
    wikidict=pickle.load(open("derek_sim_prob_dict.pkl","rb"))
    for line, x in zip(out,test):
        id= x["id"]
        answer,normpercent= PickBestAnswer(orderedlabels,line,x)
        sim=wikidict[x['id']]
        top=topicdict[x['id']]
        norm= np.asarray(normpercent)
        totalsum=norm+1.5*np.asarray(sim)+.75*np.asarray(top)
        totalsum=list(totalsum)
        #logRegDict[x["id"]]=list(normpercent)
        if 0== totalsum.index(max(totalsum)):
            answer='A'
        if 1== totalsum.index(max(totalsum)):
            answer='B'
        if 2== totalsum.index(max(totalsum)):
            answer='C'
        if 3== totalsum.index(max(totalsum)):
            answer='D'
        csvobject.writerow({"id":x["id"],"correctAnswer":answer})
    pickle.dump(logRegDict,open("logregTRAIN.p","wb"))
    ...
    correct=0
    total=0
    topicdict=pickle.load(open("topic_prob_dict.pkl","rb"))
    wikidict=pickle.load(open("sim_prob_dict.pkl","rb"))
    for line, x in zip(out,test):
        id= x["id"]
        answer,normpercent= PickBestAnswer(orderedlabels,line,x)
        sim=wikidict[x['id']]
        top=topicdict[x['id']]
        norm= np.asarray(normpercent)
        totalsum=1.5*np.asarray(sim)+norm+.75*np.asarray(top)
        totalsum=list(totalsum)
        #logRegDict[x["id"]]=list(normpercent)
        if 0== totalsum.index(max(totalsum)):
            answer='A'
    ...
    #pickle.dump(logRegDict,open("someLogRegTrain.p","wb"))