...
Using the whole summary did not work very well.  However, looking at the structure of the testing data, each test question is approximatly one sentence, while each training example is also approximatly one sentence.  To emulate this, each wikipedia summary was seperated by periods.  This gave multiple strings that most likely were a single sentence.  This was then added to the logistic regression model, as training data.
\label{ssec:log_reg}
