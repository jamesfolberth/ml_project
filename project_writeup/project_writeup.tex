...
These questions, which make up roughly 2/3 of the dataset, use the keywords "this" and "these" to indicate a general category that an answer must fit. For example 1 above the correct answer must be an organelle which immediately eliminates answers (b) and (d), while in example 2 only answer (b) fits the category of phylum and must therefor be the correct answer. In this manner we hoped to develope a system that would eliminate incorrect answers with a very low false-positive rate so that the other classifiers we implemented could achive better performance. The first step in this process involved utilizing the NLTK to  perform part-of-speech tagging on all of the questions in the dataset. These POS tags can then be used to find the noun that most immediately follows the keywords, in questions where the keywords are present, and in doing so obtain the answer category. For each question, we hoped that we would be able to utilize the PyGoogle module to make queries to Google to help determine whether an answer was a member of the necesary category for each question. The PyGoogle module has a method that returns the number of web pages found for a given query, and seemed like a good way to quantify how related an answer and category are. After fruitlessly fighting with the PyGoogle API to perform the large number of queries necessary for this method to be successful, we eventually decided to use only the Wikipedia data to eliminate answers. Through cross-validation we discovered that this did not lead to any imporved performance when used in conjunction with the other algorithms, and was not implemented for the final classification.
